...
ok('z13 vid 60°N ≈ 9.6 m/px', Math.abs(metresPerPixel(59.77, 13) - 9.62) < 0.1, metresPerPixel(59.77,13).toFixed(2))
ok('z14 vid 60°N ≈ 4.8 m/px', Math.abs(metresPerPixel(59.77, 14) - 4.81) < 0.1, metresPerPixel(59.77,14).toFixed(2))

// The zoom choice should land on the finest tile that suffices.
const box = { south: 59.755, north: 59.785, west: 17.634, east: 17.682 }
for (const [target, expected] of [[15, 13], [6, 14], [40, 11], [200, 9]] as [number, number][]) {
...
const lunsen = m.elevation(59.7697, 17.6581)
...
let prevCoarse = coarse.elevation(59.760, 17.640), prevFine = m.elevation(59.760, 17.640)
for (let i = 1; i <= 200; i++) {
  const lat = 59.760 + (i / 200) * 0.02
  const c = coarse.elevation(lat, 17.640), f = m.elevation(lat, 17.640)
